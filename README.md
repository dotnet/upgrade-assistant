...
We're interested to hearing how your experience with the .NET Upgrade Assistant has been going as you upgrade your project(s) from .NET Framework to .NET 6 and beyond.
...
This project enables automation of common tasks related to upgrading .NET Framework projects to .NET 6 and beyond. Note that this is not a complete upgrade tool and work *will* be required after using the tooling to upgrade a project.
...
- Re-target project to .NET 6.0
- Update NuGet package dependencies to versions that are compatible with .NET 6.0
- Make simple updates in C# source code to replace patterns that worked in .NET Framework with .NET 6.0 equivalents
...
- Package dependencies that need to be removed / added / upgraded in order to upgrade the project to chosen TFM (.NET 6.0 and beyond)
- References that need to be removed / added / upgraded in order to upgrade the project to chosen TFM (.NET 6.0 and beyond)
- Framework References that need to be removed / added / upgraded in order to upgrade the project to chosen TFM (.NET 6.0 and beyond)
- Call out if there is a package upgrade across major versions that could lead towards having breaking changes.
- Unsupported API for the chosen TFM (.NET 6.0 and beyond) used in the projects with pointers to recommended path forward if one is available.
...
As you upgrade projects from .NET Framework to .NET 6, it will be very useful to be familiar with relevant [porting documentation](https://docs.microsoft.com/dotnet/core/porting/).
...
The following tutorials will give you a sense of how to upgrade ASP.NET, Windows Forms, and WPF applications to .NET 5 and .NET 6 using the Upgrade Assistant:
...
Note that this tool does not (yet) advise on the feasibility or estimated cost of upgrading projects. It assumes that projects it runs on have already been reviewed and a decision taken to upgrade them to .NET 6.0.

If you're just starting to look at .NET 6.0 and would like to understand more about potential challenges in upgrading any particular project, you should begin by looking at .NET Framework dependencies the project has and third-party libraries or NuGet packages it depends on and understand whether those dependencies are likely to work on .NET 6.0. Resources that can help with that analysis include:
...
## Roadmap
Take a look at the high level overview of the roadmap for this tool and the journey to upgrade your apps from .NET Framework to .NET 6 and beyond in the [roadmap](docs/roadmap.md).

## Engage, Contribute and Give Feedback
Some of the best ways to contribute are to use the tool to upgrade your apps to .NET 6, file issues for feature-requests or bugs, join in design conversations, and make pull-requests. 
