...
## Updaters
Various services may request an implementation of `IUpdater<TUpdater>` which provides a way to update an object of type `TUpdater`. An example is a `IUpdater<ConfigFile>` that will provide udpates for configuration files (`app.config`, `web.config`).

This generic parameter can be extended by any other service to provide a custom way of defining an updater.
