...
namespace Microsoft.DotNet.UpgradeAssistant.Extensions.Default
{
    public class DefaultExtensionServiceProvider : IExtensionServiceProvider
    {
        ...
        private static void AddMigrationSteps(IServiceCollection services, IConfiguration configuration)
        {
            ...
            services.AddPackageUpdaterStep().Bind(configuration.GetSection(PackageUpdaterStepOptionsSection));
            services.AddProjectFormatSteps().Bind(configuration.GetSection(TryConvertProjectConverterStepOptionsSection));
